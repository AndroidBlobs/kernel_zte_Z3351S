...
static void cts_delayed_work_upgrade(struct work_struct *work)
{
	...
	cts_data = container_of(work, struct chipone_ts_data, upgrade_work.work);
...
static int cts_i2c_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&cts_data->upgrade_work, cts_delayed_work_upgrade);
	...
	queue_delayed_work(cts_data->workqueue, &cts_data->upgrade_work, 4 * HZ);
