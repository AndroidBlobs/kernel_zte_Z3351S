...
char ts_config_id2 = 0;
#endif
bool syna_shutdown_process = false;
...
char *syn_fwfile_table[SYN_MOUDLE_NUM_MAX] = {
	...
	SYN_LCE_FW_NAME,
	SYN_GUOXIAN_FW_NAME,
	SYN_HELITECH_FW_NAME,
	SYN_JDI_FW_NAME,
	SYN_HUAXINGDA_FW_NAME,
	SYN_TOPTOUCH_FW_NAME,
	SYN_GVO_FW_NAME,
	SYN_WALLY_PANEL_FW_NAME
};

#ifdef TP_LOG
struct touchscreen_log {
	int month;
	int day;
	int hour;
	int min;
...
#define LOG_MAX_ITEM 2000
#define NR_FINGERS	10
static struct timespec timespec;
static struct rtc_time time;
static int current_item_num;
static struct touchscreen_log ts_log[LOG_MAX_ITEM] = {};
...
extern unsigned char *customer_ID;

extern  int fwu_get_device_config_id(void);
extern int socinfo_get_ftm_flag(void);

#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_FW_UPDATE_V27)
...
#endif
extern int syna_tp_fw_upgrade(struct tpd_classdev_t *cdev, char *fw_name, int fwname_len);
...
#ifdef TP_LOG
static void synaptics_rmi4_set_log(int finger_id, int finger_status)
{
	timespec = current_kernel_time();
	timespec.tv_sec -= 60*sys_tz.tz_minuteswest;
	rtc_time_to_tm(timespec.tv_sec, &time);
	ts_log[current_item_num].month = time.tm_mon + 1;
	ts_log[current_item_num].day = time.tm_mday;
	ts_log[current_item_num].hour = time.tm_hour;
	ts_log[current_item_num].min = time.tm_min;
	ts_log[current_item_num].second = time.tm_sec;
	ts_log[current_item_num].millisecond = timespec.tv_nsec / NSEC_PER_MSEC;
	ts_log[current_item_num].finger_id = finger_id;
	ts_log[current_item_num].finger_status = finger_status;

	current_item_num++;
	if (current_item_num >= LOG_MAX_ITEM)
		current_item_num = 0;
}
#endif
...
#ifdef REPORTTING_CONTROL_8BIT
	if (enable)
		reporting_control[0] = F12_WAKEUP_GESTURE_MODE;
	else
		reporting_control[0] = F12_CONTINUOUS_MODE;
#else
	if (enable)
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (detected_gestures) {
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_DOUBLEC, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_DOUBLEC, 0);
			input_sync(rmi4_data->input_dev);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (add_log)
			synaptics_rmi4_set_log(finger, finger_status);
#endif
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		...
		set_bit(KEY_GESTURE_DOUBLEC, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_POWER);
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_DOUBLEC);
	}
}
...
#ifdef GPIO_CONTROL_LDO
static int syna_gpio_ldo_power_init(struct synaptics_rmi4_data *rmi4_data)
{
	int rc;
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;

	if ((bdata->pwr_avdd_name != NULL) && (*bdata->pwr_avdd_name != 0)) {
		rmi4_data->ldo_avdd = regulator_get(rmi4_data->pdev->dev.parent,
			bdata->pwr_avdd_name);
		if (IS_ERR(rmi4_data->ldo_avdd)) {
			rc = PTR_ERR(rmi4_data->ldo_avdd);
			pr_err("Regulator get failed ldo_avdd rc=%d", rc);
			goto avdd_regulator_put;
		}
	}
	if ((bdata->pwr_iovdd_name != NULL) && (*bdata->pwr_iovdd_name != 0)) {
		rmi4_data->ldo_iovdd = regulator_get(rmi4_data->pdev->dev.parent,
			bdata->pwr_iovdd_name);
		if (IS_ERR(rmi4_data->ldo_iovdd)) {
			rc = PTR_ERR(rmi4_data->ldo_iovdd);
			pr_err("Regulator get failed ldo_avdd rc=%d", rc);
			goto iovdd_regulator_put;
		}
	}
	return 0;

iovdd_regulator_put:
	if (rmi4_data->ldo_iovdd) {
		regulator_put(rmi4_data->ldo_iovdd);
		rmi4_data->ldo_iovdd = NULL;
	}
avdd_regulator_put:
	if (rmi4_data->ldo_avdd) {
		regulator_put(rmi4_data->ldo_avdd);
		rmi4_data->ldo_avdd = NULL;
	}
	return rc;
}
#endif
		...
		bool get)
{
	...
	#ifdef GPIO_CONTROL_LDO
	syna_gpio_ldo_power_init(rmi4_data);
	#endif
		...
		bool enable)
{
	...
	#ifdef GPIO_CONTROL_LDO
	if (rmi4_data->ldo_avdd) {
		retval = regulator_enable(rmi4_data->ldo_avdd);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to enable power ldo_avdd\n",
					__func__);
			goto disable_ldo_avdd;
		} else
			pr_notice("%s: enable power regulator of ldo_avdd successfully!\n", __func__);
	}

	if (rmi4_data->ldo_iovdd) {
		retval = regulator_enable(rmi4_data->ldo_iovdd);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to enable power ldo_iovdd\n",
					__func__);
			goto disable_ldo_iovdd;
		} else
			pr_notice("%s: enable power regulator of ldo_iovdd successfully!\n", __func__);
...
disable_ldo_iovdd:
	if (rmi4_data->ldo_iovdd) {
		retval = regulator_disable(rmi4_data->ldo_iovdd);
		if (retval < 0)
			pr_notice("%s: failed to disable regulator of ldo_iovdd!\n", __func__);
		else
			pr_notice("%s: disable regulator of ldo_iovdd successfully!\n", __func__);
	}
disable_ldo_avdd:
	if (rmi4_data->ldo_avdd) {
		retval = regulator_disable(rmi4_data->ldo_avdd);
		if (retval < 0)
			pr_notice("%s: failed to disable regulator of ldo_avdd!\n", __func__);
		else
			pr_notice("%s: disable regulator of ldo_avdd successfully!\n", __func__);
	}
#endif
...
#ifdef CONFIG_TOUCHSCREEN_SPECIAL_INTERFACE
extern int synaptics_get_f34_crtl_info(struct synaptics_rmi4_data *ts,
	unsigned int *chip_id, unsigned int *vendor_id, unsigned int *fw_version);

static int tpd_require_stay_awake(struct tpd_classdev_t *cdev)
{
	int retval = 0;
	struct synaptics_rmi4_data *synap_data = (struct synaptics_rmi4_data *) cdev->private;

	synap_data->need_stay_awake = true;
	msleep(50);
	if (synap_data->tpd_suspend) {
		pr_notice("%s: ts in suspend mode, wait 5 sec\n", __func__);
		msleep(3000);
		if (synap_data->tpd_suspend) {
			pr_notice("%s: ts still in suspend mode, resume.\n", __func__);
			synap_data->need_stay_awake = false;
			retval = -1;
		}
		synap_data->need_stay_awake = true;
	}
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	strlcpy(cdev->ic_tpinfo.tp_name, "Synaptics", sizeof(cdev->ic_tpinfo.tp_name));
...
static int syna_i2c_reg_read(struct tpd_classdev_t *cdev, char addr, u8 *data, int len)
{
	struct synaptics_rmi4_data *ts = (struct synaptics_rmi4_data *) cdev->private;

	return synaptics_rmi4_reg_read(ts, addr, data, len);
}

static int syna_i2c_reg_write(struct tpd_classdev_t *cdev, char addr, u8 *data, int len)
{
	struct synaptics_rmi4_data *ts = (struct synaptics_rmi4_data *) cdev->private;

	return synaptics_rmi4_reg_write(ts, addr, data, len);
}
...
static int tpd_get_wakegesture(struct tpd_classdev_t *cdev)
{
	...
	cdev->b_gesture_enable = rmi4_data->enable_wakeup_gesture;
	return 0;
}
...
static int tpd_enable_wakegesture(struct tpd_classdev_t *cdev, int enable)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->enable_wakeup_gesture = enable;
...
static int tpd_get_smart_cover(struct tpd_classdev_t *cdev)
{
	...
	cdev->b_smart_cover_enable = rmi4_data->enable_smart_cover;
	return 0;
}

static int tpd_enable_smart_cover(struct tpd_classdev_t *cdev, int enable)
{
	struct synaptics_rmi4_data *rmi4_data = (struct synaptics_rmi4_data  *)cdev->private;

	rmi4_data->enable_smart_cover = enable;
	pr_notice("%s: rmi4_data->enable_smart_cover=%d\n", __func__, rmi4_data->enable_smart_cover);
	return enable;
}

static int syna_gpio_shutdown_config(void)
{
	syna_shutdown_process = true;
	queue_work(syna_rmi4_resume_wq, &syna_rmi4_suspend_work);
	suspend_flag = 0;
	wait_event_interruptible_timeout(suspend_wait, suspend_flag, HZ);
	pr_notice("%s: tp shutdown,suspend.\n", __func__);
	syna_shutdown_process = false;
	return 0;
}

static int syna_tp_suspend_show(struct tpd_classdev_t *cdev)
{
	struct synaptics_rmi4_data *rmi4_data = (struct synaptics_rmi4_data  *)cdev->private;

	cdev->tp_suspend = rmi4_data->suspend;
	return cdev->tp_suspend;
}

static int syna_set_tp_suspend(struct tpd_classdev_t *cdev, int enable)
{
	int ret = 0;
	struct synaptics_rmi4_data *rmi4_data = (struct synaptics_rmi4_data  *)cdev->private;
	...
	if (enable) {
		...
		pr_notice("syna_set_tp_suspend: ret=%u, HZ=%d\n", ret, HZ);
	} else {
...
static int tpd_register_fw_class(struct synaptics_rmi4_data *data)
{
	...
	tpd_fw_cdev.tp_i2c_reg_read = syna_i2c_reg_read;
	tpd_fw_cdev.tp_i2c_reg_write = syna_i2c_reg_write;
	...
	tpd_fw_cdev.set_smart_cover = tpd_enable_smart_cover;
	tpd_fw_cdev.tp_fw_upgrade = syna_tp_fw_upgrade;
	tpd_fw_cdev.tpd_gpio_shutdown = syna_gpio_shutdown_config;
	tpd_fw_cdev.tp_suspend_show = syna_tp_suspend_show;
	tpd_fw_cdev.set_tp_suspend = syna_set_tp_suspend;
...
{
	int ret;
	unsigned char fw_origin_id = 0;
	#ifdef FW_UPGRADE_BY_PANEL_NAME
	...
	if (!bootloader_mode) {
		...
		#ifdef ONE_TP_MULTI_MODULE
		ts_config_id2 = ts->config_id.fw_ver & 0x00ff;
		pr_notice("%s: ts_config_id2 = 0x%x\n", __func__, ts_config_id2);
		#endif
	} else {
		dev_info(ts->pdev->dev.parent,
	...
	case 'U':
		...
		break;
	case 'X':
		snprintf(p_chip_type, CHIP_INFO_LEN, "TD4310(0x%x)", ts->config_id.chip_type);
		break;
	default:
	...
	case 'E':
		...
		break;
	case 'F':
		snprintf(p_sensor, CHIP_INFO_LEN, "BOE(0x%x)", ts->config_id.sensor);
		touch_module = BOE;
		break;
	case 'H':
		...
		break;
	case 'I':
		snprintf(p_sensor, CHIP_INFO_LEN, "DIJING(0x%x)", ts->config_id.sensor);
		touch_module = DIJING;
		break;
	case 'P':
		snprintf(p_sensor, CHIP_INFO_LEN, "LCE(0x%x)", ts->config_id.sensor);
		touch_module = LCE;
		break;
	case 'R':
		snprintf(p_sensor, CHIP_INFO_LEN, "HELITECH(0x%x)", ts->config_id.sensor);
		touch_module = HELITECH;
		break;
	case 'S':
		snprintf(p_sensor, CHIP_INFO_LEN, "JDI(0x%x)", ts->config_id.sensor);
		touch_module = JDI;
		break;
	case 'T':
		snprintf(p_sensor, CHIP_INFO_LEN, "HUAXINGDA(0x%x)", ts->config_id.sensor);
		touch_module = HUAXINGDA;
		break;
	case 'U':
		snprintf(p_sensor, CHIP_INFO_LEN, "TOPTOUCH(0x%x)", ts->config_id.sensor);
		touch_module = TOPTOUCH;
		break;
	case 'V':
		snprintf(p_sensor, CHIP_INFO_LEN, "GVO(0x%x)", ts->config_id.sensor);
		touch_module = GVO;
		break;
	case 'W':
		snprintf(p_sensor, CHIP_INFO_LEN, "WALLY_PANEL(0x%x)", ts->config_id.sensor);
		touch_module = WALLY_PANEL;
		break;
	default:
	...
	*p_fw_ver = ts->config_id.fw_ver;
	fw_origin_id = ts->config_id.fw_ver & 0x00ff;
	pr_info("%s: fw_origin_id_1st = 0x%x\n", __func__, fw_origin_id);
#ifndef ONE_TP_MULTI_MODULE
	...
	if (strnstr(panel_name, VENDOR_ID1_NAME, strlen(panel_name)))
		syna_file_name = SYN_VENDOR1_FW_NAME;
	else if (strnstr(panel_name, VENDOR_ID2_NAME, strlen(panel_name))) {
			syna_file_name = SYN_VENDOR2_FW_NAME;
			if (strnstr(panel_name, SYN_VENDOR2_LCD_NAME, strlen(panel_name)))
				syna_file_name = SYN_VENDOR2_LCD_FW_NAME;
		}
	else if (strnstr(panel_name, VENDOR_ID3_NAME, strlen(panel_name)))
		syna_file_name = SYN_VENDOR3_FW_NAME;
	else
		pr_err("%s: get syna_file_name by panel name error.\n", __func__);
#else
	if (!bootloader_mode) {
		if (ts_config_id2 >= SYNA_VENDOR_ID1 && ts_config_id2 < SYNA_VENDOR_ID2)
			syna_file_name = SYN_VENDOR1_FW_NAME;
		else if (ts_config_id2 >= SYNA_VENDOR_ID2 && ts_config_id2 < SYNA_VENDOR_ID3)
			syna_file_name = SYN_VENDOR2_FW_NAME;
		else if (ts_config_id2 >= SYNA_VENDOR_ID3)
			syna_file_name = SYN_VENDOR3_FW_NAME;
		else
			pr_err("%s: get syna_file_name by panel name error.\n", __func__);
	} else {
		if (ts_config_id2 == SYNA_BL_ID1)
			syna_file_name = SYN_VENDOR1_FW_NAME;
		else if (ts_config_id2 == SYNA_BL_ID2)
			syna_file_name = SYN_VENDOR2_FW_NAME;
		else if (ts_config_id2 == SYNA_BL_ID3)
			syna_file_name = SYN_VENDOR3_FW_NAME;
		else
			pr_err("%s: get syna_file_name by panel name error.\n", __func__);
	}
...
#ifdef TP_LOG
ssize_t proc_read_log_val(struct file *file, char __user *page,
	size_t size, loff_t *ppos)
{
	int len = 0;
	int valid_count = 0;
	int i = current_item_num;
	int j = 1;

	if (*ppos)
		return 0;

	len += snprintf(page + len, PAGE_SIZE, "### START\n");

	for (; j <= LOG_MAX_ITEM; i++, j++) {
		if (i >= LOG_MAX_ITEM)
			i = 0;
		if ((ts_log[i].month != 0) && (ts_log[i].day != 0)) {
			valid_count++;
			len += snprintf(page + len, PAGE_SIZE,
						"%02d-%02d %02d:%02d:%02d.%03d id:%d-%s\n",
						ts_log[i].month,
						ts_log[i].day,
						ts_log[i].hour,
						ts_log[i].min,
						ts_log[i].second,
						ts_log[i].millisecond,
						ts_log[i].finger_id,
						(ts_log[i].finger_status == 1) ? "down" : "up");
		}
	}

	len += snprintf(page + len, PAGE_SIZE, "### END (count = %d)\n", valid_count);
...
static const struct file_operations proc_ops_read_log = {
	.owner = THIS_MODULE,
	.read = proc_read_log_val,
};
#endif
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (!retval && rmi4_data->ts_pinctrl) {
		...
		if (retval < 0)
			pr_notice("%s: Failed to select pin to active state\n", __func__);
	}
...
#if defined(CONFIG_ADF)
	rmi4_data->adf_notifier.notifier_call = ts_adf_event_handler;
	/* make sure we're the first to suspend/resume */
	rmi4_data->adf_notifier.priority = 1000;
	retval = adf_register_client(&rmi4_data->adf_notifier);
	if (retval < 0)
		dev_err(&pdev->dev, "register adf notifier fail, cannot sleep when screen off\n");
	else
		pr_notice("synaptics tp register adf notifier succeed\n");
#elif defined(CONFIG_FB)
...
err_enable_irq:
#if defined(CONFIG_ADF)
	adf_unregister_client(&rmi4_data->adf_notifier);
#elif defined(CONFIG_FB)
...
static int ts_adf_event_handler(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
			container_of(nb, struct synaptics_rmi4_data, adf_notifier);

	if (action != ADF_EVENT_BLANK)
		return NOTIFY_DONE;

	adf_event_data = *(int *)event->data;
	pr_notice("receive adf event with adf_event_data=%d", adf_event_data);
	...
	case DRM_MODE_DPMS_OFF:
		...
		break;
	default:
		pr_err("receive adf event with error data, adf_event_data=%d",
			adf_event_data);
		break;
	}
	return NOTIFY_OK;
	...
	if (rmi4_data->enable_wakeup_gesture) {
		if (rmi4_data->no_sleep_setting) {
			ret = synaptics_rmi4_reg_read(rmi4_data,
					...
					sizeof(device_ctrl));
			if (ret < 0) {
				pr_err("%s: failed to get device_ctrl\n", __func__);
			}
			...
			ret = synaptics_rmi4_reg_write(rmi4_data,
					...
					sizeof(device_ctrl));
			if (ret < 0) {
				pr_err("%s: failed to write device_ctrl\n", __func__);
			}
		}
...
static void synaptics_rmi4_suspend(struct work_struct *work)
{
	...
	if (!rmi4_data->suspend) {
		...
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
		usleep_range(TDDI_LPWG_WAIT_US, TDDI_LPWG_WAIT_US + 10);
#endif
		synaptics_rmi4_irq_enable(rmi4_data, false, false);
		synaptics_rmi4_sleep_enable(rmi4_data, true);
		synaptics_rmi4_free_fingers(rmi4_data);
	...
	rmi4_data->tpd_suspend = true;
	if (rmi4_data->need_stay_awake)
		TPD_DMESG("%s: tpd need_stay_awake.\n", __func__);
#endif
