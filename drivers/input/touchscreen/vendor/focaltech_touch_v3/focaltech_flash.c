...
***********************************************************************/
static int fts_fwupg_get_boot_state(struct fts_upgrade *upg, enum FW_STATUS *fw_sts)
{
	int ret = 0;
	u8 cmd[4] = { 0 };
	u32 cmd_len = 0;
...
static int fts_pram_ecc_cal_algo(struct fts_upgrade *upg, u32 start_addr, u32 ecc_length)
{
	int ret = 0;
	int i = 0;
	int ecc = 0;
...
int fts_read_file(char *file_name, u8 **file_buf)
{
	...
	if (IS_ERR(filp)) {
		FTS_ERROR("open %s file fail, try open /vendor/firmware/", file_path);
		snprintf(file_path, FILE_NAME_LENGTH, "%s%s", "/vendor/firmware/", file_name);
		filp = filp_open(file_path, O_RDONLY, 0);
		if (IS_ERR(filp)) {
...
static int fts_fwupg_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
	...
	if (upg->fw_length < upg->func->fwveroff) {
		FTS_ERROR("fw len(0x%0x) < fw ver offset(0x%x)", upg->fw_length, upg->func->fwveroff);
		return -EINVAL;
	}
