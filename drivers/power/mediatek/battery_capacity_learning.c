...
static int batt_cap_learn_linearization_topoff(struct batt_cap_learn_info *chip)
{
	...
	for (i = 0; i < battery_config->topoff_prop_step - 1; i++) {
		if ((age <= battery_config->topoff_prop[i].topoff_age)
			&& (age > battery_config->topoff_prop[i + 1].topoff_age)) {
			break;
		}
	}

	if (i >= battery_config->topoff_prop_step - 1) { /*last step, Not have "last + 1" level*/
		pr_alarm("age[%d] less than [%d],  reset level\n", age, battery_config->topoff_prop[i].topoff_age);
		i--;
	}

	pr_alarm("[age -> capacity(topoff)] x1: %d(age); x2: %d(age); y1: %d(mAH); y2: %d(mAH); x: %d(age)\n",
							battery_config->topoff_prop[i].topoff_age,
							battery_config->topoff_prop[i + 1].topoff_age,
							battery_config->topoff_prop[i].topoff_capacity_delta,
							battery_config->topoff_prop[i + 1].topoff_capacity_delta,
							age);

	linear_capacity = linearization_handle(battery_config->topoff_prop[i].topoff_age,
							battery_config->topoff_prop[i + 1].topoff_age,
							battery_config->topoff_prop[i].topoff_capacity_delta,
							battery_config->topoff_prop[i + 1].topoff_capacity_delta,
							age);
...
static int batt_cap_learn_linearization_reduce_voltage(struct batt_cap_learn_info *chip)
{
	...
	for (i = 0; i < battery_config->reduc_volt_prop_step - 1; i++) {
		if ((linear_prop.reduc_age <= battery_config->reduc_volt_prop[i].reduc_age)
			&& (linear_prop.reduc_age > battery_config->reduc_volt_prop[i + 1].reduc_age)) {
			break;
		}
	}

	if (i >= battery_config->reduc_volt_prop_step - 1) { /*last step, Not have "last + 1" level*/
		pr_alarm("age[%d] less than [%d],  reset level\n", linear_prop.reduc_age,
				battery_config->reduc_volt_prop[i].reduc_age);
		i--;
	}

	pr_alarm("[age -> 50_capacity] x1: %d(age); x2: %d(age); y1: %d(mAH); y2: %d(mAH); x: %d(V)\n",
						battery_config->reduc_volt_prop[i].reduc_age,
						battery_config->reduc_volt_prop[i + 1].reduc_age,
						battery_config->reduc_volt_prop[i].reduc_volt50_capacity,
						battery_config->reduc_volt_prop[i + 1].reduc_volt50_capacity,
						linear_prop.reduc_age);
	linear_prop.reduc_volt50_capacity = linearization_handle(battery_config->reduc_volt_prop[i].reduc_age,
						battery_config->reduc_volt_prop[i + 1].reduc_age,
						battery_config->reduc_volt_prop[i].reduc_volt50_capacity,
						battery_config->reduc_volt_prop[i + 1].reduc_volt50_capacity,
						linear_prop.reduc_age);
	...
	pr_alarm("[age -> 100_capacity] x1: %d(age); x2: %d(age); y1: %d(mAH); y2: %d(mAH); x: %d(V)\n",
						battery_config->reduc_volt_prop[i].reduc_age,
						battery_config->reduc_volt_prop[i + 1].reduc_age,
						battery_config->reduc_volt_prop[i].reduc_volt100_capacity,
						battery_config->reduc_volt_prop[i + 1].reduc_volt100_capacity,
						linear_prop.reduc_age);
	linear_prop.reduc_volt100_capacity = linearization_handle(battery_config->reduc_volt_prop[i].reduc_age,
						battery_config->reduc_volt_prop[i + 1].reduc_age,
						battery_config->reduc_volt_prop[i].reduc_volt100_capacity,
						battery_config->reduc_volt_prop[i + 1].reduc_volt100_capacity,
						linear_prop.reduc_age);
...
static int batt_cap_learn_linearization_disch_volt_curr(struct batt_cap_learn_info *chip)
{
	...
	for (i = 0; i < battery_config->disch_curr_volt_prop_step - 1; i++) {
		if ((linear_prop.disch_age <= battery_config->disch_curr_volt_prop[i].disch_age)
			&& (linear_prop.disch_age > battery_config->disch_curr_volt_prop[i + 1].disch_age)) {
			break;
		}
	}

	if (i >= battery_config->disch_curr_volt_prop_step - 1) { /*last step, Not have "last + 1" level*/
		pr_alarm("age[%d] less than [%d],  reset level\n", linear_prop.disch_age,
				battery_config->disch_curr_volt_prop[i].disch_age);
		i--;
	}

	pr_alarm("[age_now -> 150mA_capacity] x1: %d(age); x2: %d(age); y1: %d(mAH); y2: %d(mAH); x: %d(age)\n",
				battery_config->disch_curr_volt_prop[i].disch_age,
				battery_config->disch_curr_volt_prop[i + 1].disch_age,
				battery_config->disch_curr_volt_prop[i].disch_capacity_curr_150,
				battery_config->disch_curr_volt_prop[i + 1].disch_capacity_curr_150,
				linear_prop.disch_age);
	linear_prop.disch_capacity_curr_150 = linearization_handle(battery_config->disch_curr_volt_prop[i].disch_age,
				battery_config->disch_curr_volt_prop[i + 1].disch_age,
				battery_config->disch_curr_volt_prop[i].disch_capacity_curr_150,
				battery_config->disch_curr_volt_prop[i + 1].disch_capacity_curr_150,
				linear_prop.disch_age);
	...
	pr_alarm("[age_now -> 0.02C_capacity] x1: %d(age); x2: %d(age); y1: %d(mAH); y2: %d(mAH); x: %d(age)\n",
				battery_config->disch_curr_volt_prop[i].disch_age,
				battery_config->disch_curr_volt_prop[i + 1].disch_age,
				battery_config->disch_curr_volt_prop[i].disch_capacity_curr_0d2c,
				battery_config->disch_curr_volt_prop[i + 1].disch_capacity_curr_0d2c,
				linear_prop.disch_age);

	linear_prop.disch_capacity_curr_0d2c = linearization_handle(battery_config->disch_curr_volt_prop[i].disch_age,
				battery_config->disch_curr_volt_prop[i + 1].disch_age,
				battery_config->disch_curr_volt_prop[i].disch_capacity_curr_0d2c,
				battery_config->disch_curr_volt_prop[i + 1].disch_capacity_curr_0d2c,
				linear_prop.disch_age);
	...
	pr_alarm("[age_now -> 1300mA_capacity] x1: %d(age); x2: %d(age); y1: %d(mAH); y2: %d(mAH); x: %d(age)\n",
				battery_config->disch_curr_volt_prop[i].disch_age,
				battery_config->disch_curr_volt_prop[i + 1].disch_age,
				battery_config->disch_curr_volt_prop[i].disch_capacity_curr_1300,
				battery_config->disch_curr_volt_prop[i + 1].disch_capacity_curr_1300,
				linear_prop.disch_age);

	linear_prop.disch_capacity_curr_1300 = linearization_handle(battery_config->disch_curr_volt_prop[i].disch_age,
				battery_config->disch_curr_volt_prop[i + 1].disch_age,
				battery_config->disch_curr_volt_prop[i].disch_capacity_curr_1300,
				battery_config->disch_curr_volt_prop[i + 1].disch_capacity_curr_1300,
				linear_prop.disch_age);
...
static int batt_cap_learn_linearization_disch_volt(struct batt_cap_learn_info *chip)
{
	...
	linear_capacity = linearization_handle(battery_config->disch_curr_volt_prop[i].disch_age,
				battery_config->disch_curr_volt_prop[i + 1].disch_age,
				battery_config->disch_curr_volt_prop[i].disch_capacity_curr_0d2c,
				battery_config->disch_curr_volt_prop[i + 1].disch_capacity_curr_0d2c,
				linear_prop.disch_age);
					...
					struct device_node *np, int index)
{
	...
	OF_READ_ARRAY_PROPERTY(battery_config->topoff_prop,
						battery_config->topoff_prop_step,
						sizeof(struct battery_topoff_prop),
						"topoff-prop", retval);

	for (i = 0; i < battery_config->topoff_prop_step; i++) {
		pr_alarm("topoff-prop data:%d\t%d\n", battery_config->topoff_prop[i].topoff_age,
				battery_config->topoff_prop[i].topoff_capacity_delta);
	}

	OF_READ_ARRAY_PROPERTY(battery_config->disch_curr_volt_prop,
						battery_config->disch_curr_volt_prop_step,
						sizeof(struct battery_disch_curr_volt_prop),
						"disch-curr-volt-prop", retval);

	for (i = 0; i < battery_config->disch_curr_volt_prop_step; i++) {
	...
	OF_READ_ARRAY_PROPERTY(battery_config->reduc_volt_prop,
						battery_config->reduc_volt_prop_step,
						sizeof(struct battery_reduc_volt_prop),
						"reduc-volt-prop", retval);

	for (i = 0; i < battery_config->reduc_volt_prop_step; i++) {
...
parse_dt_failed:
	for (i = 0; i < chip->config_par.battery_num; i++) {
		kfree(chip->config_par.battery_info[i].topoff_prop);
		kfree(chip->config_par.battery_info[i].disch_curr_volt_prop);
		kfree(chip->config_par.battery_info[i].reduc_volt_prop);
	}
