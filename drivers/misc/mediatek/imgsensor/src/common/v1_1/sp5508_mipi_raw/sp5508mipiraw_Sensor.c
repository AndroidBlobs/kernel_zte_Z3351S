...
u8 sp5508MIPI_Initialize_from_T_Flash(void)
{
	...
	while (curr_ptr < (data_buff + file_size)) {
		...
		if (strcmp((const char *)func_ind, "REG") == 0) {
			...
			sp5508MIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 10);
		}
		i++;
	...
	for (j = 0; j < i; j++) {
		if (sp5508MIPI_Init_Reg[j].op_code == sp5508MIPI_OP_CODE_END) {
			/* End of the setting. */
			break;
